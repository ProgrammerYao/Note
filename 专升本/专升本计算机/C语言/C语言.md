...
* 补码求原码：
	* 正数不变；
	* 负数逆运行即可(-1再取反)
* 注意：负数的补码转换为十进制数时，符号是根据补码的，而值是原码
...
* 单引号(' ')括起的 单个普通字符 或 转义字符
	* 常用ASCII码：'A'-65，'Z'-112，'a'-97，'z'-90，'0'-48，'\0'-0（大写+32=小写）、（字符数字-48=int数字）
	* 注意：字符参与运算是根据ASCLL计算的。例：`'0'`计算时为48，而`0`计算时为0
	* 重要转义字符：
...
全局变量定义时就有初始值为零；局部变量定义时也有初始值但为随机数，除非使用了`static`修饰
...
* 内部变量，仅允许当前文件使用
* 注意：重复执行函数时重新定义静态局部变量的同语句无效，但其它重定义语句有效
...
* char类型既能表示字符也能表示字节，当为`'内容'`赋值时为字符，直接的数值赋值为字节，内容不能是中文，因为2或3个字节内容不能赋值给1字节的char，中文要用字符数组
...
* 数组存储大小：一个元素所占字节×元素个数(为下标数)

初始化：`数据类型 数组名[下标] = {元素值表列};`

* 当元素表列中元素个数确定时，可以不写数组下标
* 注意：
	* 初始化不能拆分为定义和赋值，因为引用每次只能引用一个元素
	* 赋值元素的数量≤数组长度，不能大于
* 自动初始化：初始化时当元素初始化元素>数组长度，剩余元素会自动初始化0值，整数-0，浮点数-0.0，字符-'\\0'
...
| 函数                 | 功能      | 说明                                                                          | 例子                                                           |
| ------------------ | ------- | --------------------------------------------------------------------------- | ------------------------------------------------------------ |
| strlen(str)        | 获取字符串长度 | 会统计转义字符，不会统计结束标记符'\0'，统计到'\0'结束，并且八进制数的'\0'比结束标记符优先级高                       | `strlen("'\t\"\xff\065\n")=6`<br>`strlen("hello\0\t\'\\")=9` |
...
指针变量：存储了内存地址(首地址)的变量，指向变量，大小取决于编译环境，32位占用4个字节，64位占用8个字节

* 理解：指针变量存储了谁的地址，就是指向谁

指针变量定义：`数据类型 *指针变量名;`

* 数据类型：定义的指针类型应该与存储内存的变量类型一样
...
数组名退化：数组名直接使用是会退化为**指向该数组中首元素的指针**，即数组的首地址。例：一维数组名退化的首地址表示数组中的**首元素的地址**；二维数组名退化的首地址表示第一个行元素的**整行地址**(此时与第0列元素的地址值同)。因此，数组名可以作为一个地址使用

没有退化：如果没有退化则会表示**整个数组**，使用`sizeof` 运算符和地址运算符 `&`时不会退化。例：`&一维数组`和`&二维数组`都表示整个数组

==指针运算(步长)==

步长：用于地址的偏移，一个步长相当于指针移动一次，至于走了多少字节，与所指向的类型有关(变量、数组、地址)
...
注意：指针可以单个进行增减，但两个指针之间只能进行互减，此时结果表示该两指针的间隔步长，此外还能比较大小
...
* `[数组长度]`：为所要存储的数组地址的数组长度。存储为二维数组时是列元素的长度
* `数组地址`：为二维数组名，如果是一维数组要写为`&一维数组名`
* 解引用：一次解引用获得整个数组，二次解引用获得数组中的元素
...
含义：存储了地址作为元素的数组，指向变量，本质是一个数组

初始化：`数据类型 *数组名[长度] = {值1,值2,...};` #C 

* `[长度]`：表示这个数组中能够存储的指针元素个数
* 解引用：一次解引用为元素的地址，两次解引用才为内容。例：字符类型的指针数组解引用一次为整个字符串，解引用两次时为字符
* 步长：没有解引用时一步为一个数组元素；字符型的解引用一次后一步为一个字符
* 注意：当数据类型为字符时，数组存储的是各个字符串中首个字符的地址，而不是整个元素的地址
* 作用：存储若干个字符串。例：`char *strs[4] = {"abcd", "efgh", "ijkl", ...};`

区分定义数组指针与指针数组：符号结合优先级：`( ) > [ ] > *`（函数中`()`看做`{}`），哪个先结合本质就是谁，且命名时在后面
...
字符串如果以内容存储，则是整个字符串进行存储(字符串数组)；如果以地址存储，则是以第一个字符的地址进行存储(指针数组、字符串指针)

## 字符数组与字符串

==字符数组与字符串==

字符串是字符数组，但字符数组不一定是字符串，只有当字符数组中有`'\0'`才为字符串
...
* 区别：
	* 只要是`{'',''}`为字符数组，`""`或`{""}`为字符串
	* 字符串的后面必有`'\0'`填补必须占一个元素长度，字符数组内容不够时才填补`'\0'`为字符串
* 注意：不能进行比较(\=\=只比较地址)
* 字符串长度计算：
	* 初始化时有长度，直接=长度
	* 英文、符号、数字占用一个字节
...
* 字符串指针名本质：存储了字符串的第一个字符的地址

==字符串的输入与输出

输入：字符的输入需要`&`符号，字符串(字符数组)不需要

字符形式的输出仍需要解引用，因为需要提供的是变量

字符输出格式：`printf("%c",*字符串指针|*字符数组名);`

字符串的输出与其它不同，它需要提供的是地址而不是变量

字符串输出格式：`printf("%s",字符数组名|字符串指针名);`

* 例：`char *p="abcdABCD"; printf("%s",p+4);`结果为ABCD
* 注意：字符串的输出到`'\0'`前一个结束。例：`char c[5]={'a','b','\0','c','\0'}; printf("%s",c);`，结果为ab

==字符串的输入和输出函数==

字符串输入函数：`gets(字符串数组名|字符串指针名)`
...
| 函数                 | 功能      | 说明                                                                          | 例子                                                           |
| ------------------ | ------- | --------------------------------------------------------------------------- | ------------------------------------------------------------ |
| strlen(str)        | 获取字符串长度 | 不会统计结束标记符'\0'，统计到'\0'结束，会统计转义字符，并且八进制数的'\0'比结束标记符优先级高                       | `strlen("'\t\"\xff\065\n")=6`<br>`strlen("hello\0\t\'\\")=9` |
| strcpy(str1,str2)  | 复制字符串   | 将str2内容拷贝覆盖到str1，str1的长度必须大于str2内容长度                                        |                                                              |
| strcat(str1,str2)  | 连接字符串   | 拼接str2到str1后，str1的长度必须满足总拼接长，str1内容为str1纯内容+str2纯内容+`'\0'`                  |                                                              |
| strcmp(str1,strt2) | 比较字符串   | 自左向右逐个字符按 ASCII 码值大小进行比较。如果str1=str 2，返回0；如果str1>str2，返回1；如果str1<str2，返回-1。 |                                                              |
...
先说明结构体类型，才能定义结构体变量；此外也可以直接定义结构体变量
...
* 注意：共用体一次只能赋值一个成员，多次对共用体成员赋值时，共用体只能存放最后一次存入的成员的值
...
* 使用：fgets()用于字符串的，而fread用于任意类型数据，通过字节数控制
* 注意：
	* 每次读写操作都会使文件位置指向变化，并且是共用的，使下一次读取不是从头开始，只有重新关闭再打开文件或`rewind()`会刷新到文件首
	* `fgetc()` /  `fgets()` / `fread()`在读取内容时，如果原来字符或数组有内容会进行从头覆盖，并且数组从头覆盖时如果后面部分没有被覆盖则会按原来输出而不是为空
	* `fscanf()`读取时，会读取到不同的格式化内容或换行符才结束。例：`fprintf()`三次分别写入7、8、9，而`fscanf(fp,"%d",&n)`会直接读取三次使n=789
...
思路：假设该数为(int)num，则循环执行判断num%(2~num-1)是否为0
...
排序思路：从第一位开始循环，另一个指针向后走，内循环找到最小字符，判断该字符是否为外循环当位，如果是则不需要交换，不是则进行交换
